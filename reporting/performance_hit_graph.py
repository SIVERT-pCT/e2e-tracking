...
for j, name in enumerate(["pat_lambda_25", "pat_lambda_50", "pat_lambda_75", "ptt"]):
    exp = ExperimentBase.from_file(f"experiments/hit_graph/{name}.json")
    experiment_to_table(exp, f"tables/{name}_hit_graph.tex")
...
for a in ax[:,0]: a.set_ylabel("True pos. rate [%]"); a.set_ylim([93.3, 99.5]); a.ticklabel_format(style='sci', axis='x', scilimits=(0,0)); a.set_xticks([0, 10000, 20000, 30000])
for a in ax[:,1]: a.set_ylabel("False pos. rate [%]"); a.set_ylim([0.25, 1.5]); a.ticklabel_format(style='sci', axis='x', scilimits=(0,0)); a.set_xticks([0, 10000, 20000, 30000])
for a in ax[:,2]: a.set_ylabel("Track purity [%]"); a.set_ylim([36, 86]); a.ticklabel_format(style='sci', axis='x', scilimits=(0,0)); a.set_xticks([0, 10000, 20000, 30000])
for a in ax[:,3]: a.set_ylabel("Track efficiency [%]"); a.set_ylim([36, 75]); a.ticklabel_format(style='sci', axis='x', scilimits=(0,0)); a.set_xticks([0, 10000, 20000, 30000])

for j, (name, label) in enumerate(zip(["pat_lambda_25", "pat_lambda_50", "pat_lambda_75", "ptt"],
                                      [r"PAT ($\lambda = 25$)",r"PAT ($\lambda = 50$)",r"PAT ($\lambda = 75$)",r"PTT"])):

    exp = ExperimentBase.from_file(f"experiments/hit_graph/{name}.json")

    val_pur, val_eff, val_tpr, val_fpr = create_n_buffer(4, 5, 300)
    train_pur, train_eff, train_tpr, train_fpr = create_n_buffer(4, 5, 300)
    ...
    ax[0, 0].plot(100 * np.arange(train_tpr.shape[1]), train_tpr.mean(axis=0), linewidth=1, color=colors[j], label=label)
    ...
    ax[0, 1].plot(100 * np.arange(train_fpr.shape[1]), train_fpr.mean(axis=0), linewidth=1, color=colors[j], label=label)
    ...
    ax[0, 2].plot(100 * np.arange(train_pur.shape[1]), train_pur.mean(axis=0), linewidth=1, color=colors[j], label=label)
    ...
    ax[0, 3].plot(100 * np.arange(train_eff.shape[1]), train_eff.mean(axis=0), linewidth=1, color=colors[j], label=label)
    ...
    ax[1, 0].plot(100 * np.arange(val_tpr.shape[1]), val_tpr.mean(axis=0), linewidth=1, color=colors[j], label=label)
    ...
    ax[1, 1].plot(100 * np.arange(val_fpr.shape[1]), val_fpr.mean(axis=0), linewidth=1, color=colors[j], label=label)
    ...
    ax[1, 2].plot(100 * np.arange(val_pur.shape[1]), val_pur.mean(axis=0), linewidth=1, color=colors[j], label=label)
    ...
    ax[1, 3].plot(100 * np.arange(val_eff.shape[1]), val_eff.mean(axis=0), linewidth=1, color=colors[j], label=label)
...
plt.tight_layout()
plt.savefig("figures/training_curve_hit_graph.pdf")