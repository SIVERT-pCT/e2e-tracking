...
def compare_algorithms_to_table(def_exp: ExperimentBase, output: str):
    df_res = pd.DataFrame()
...
def ablation_to_table(def_exp: ExperimentBase, abl_exp: ExperimentBase, output: str):
    df_res = pd.DataFrame()
    df_grad = pd.DataFrame()

    for wpt in [100, 150, 200]:
        get_file = lambda exp, wpt: f"{exp.model.model_dir}/results_water_{wpt}.pkl"
...
def experiment_to_table(exp: ExperimentBase, output: str):
    df_res = pd.DataFrame()
