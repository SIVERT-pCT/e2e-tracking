...
from src.loss_landscape.hessian.utils import group_product, group_add, normalization, get_params_grad, hessian_vector_product, orthnormal
...
class Hessian:
    ...
    def __init__(self, model: nn.Module, transforms: CustomGraphTransform,  
                 solver: Optional[CombinatorialSolver], dataset: HitGraphDataset, 
                 device: str):
        """
    ...
    def dataset_hv_product(self, v):
        ...
        for i, G in enumerate(self.dataset):
            ...
            edge_probs = torch.sigmoid(self.model(LG))

            if self.solver != None:
                edge_probs = self.solver(HG, edge_probs, train=False)
    ...
    def eigenvalues(self, maxIter=100, tol=1e-3, top_n=1):
        ...
        while computed_dim < top_n:
            ...
            for i in tqdm.tqdm(range(maxIter)):
                ...
                tmp_eigenvalue, Hv = self.dataset_hv_product(v)
                v = normalization(Hv)
    ...
    def trace(self, maxIter=100, tol=1e-3):
        ...
        for i in range(maxIter):
            ...
            _, Hv = self.dataset_hv_product(v)
    ...
    def density(self, iter=100, n_v=1):
        ...
        for k in range(n_v):
            ...
            for i in range(iter):
                ...
                if i == 0:
                    _, w_prime = self.dataset_hv_product(v)
